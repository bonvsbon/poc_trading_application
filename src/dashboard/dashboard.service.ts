...
export class DashboardService {
  ...
  private readonly news: NewsEvent[] = [
  ...
  approveSignal(id: string): DashboardState {
    if (this.tradingHalted) {
      throw new ConflictException('Trading is halted: cannot approve signals');
    }
  ...
  private updateSignalStatus(id: string, status: TradingSignal['status']) {
    const signal = this.findSignal(id);
    this.signals = this.signals.map((item) =>
      item.id === signal.id ? { ...item, status, priority: false } : item,
    );
  }

  private findSignal(id: string): TradingSignal {
    const signal = this.signals.find((item) => item.id === id);
    if (!signal) {
      throw new NotFoundException(`Signal ${id} was not found`);
    }
    return signal;
  }
}
